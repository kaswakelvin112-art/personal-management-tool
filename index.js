...
function updateLiveActivity() {
  ...
  if (nextActivity) {
    ...
    if (timeToNext > 0 && timeToNext <= 300000) {
      document.getElementById("nextActivity").style.background = "#fef3c7";
...
function checkReminders() {
  ...
  scheduleData.forEach((item) => {
    if (item.Day === currentDay && item.EnableReminder) {
      ...
      reminderDateTime.setHours(startHours, startMinutes - reminderMinutes, 0);
...
function updateAnalytics() {
  ...
  const uniqueDays = [...new Set(scheduleData.map((item) => item.Day))].length; //assures a single copy
...
function resetToDefault() {
  if (
    confirm(
      "This will replace your current schedule with the default one. Continue?",
    )
  ) {
    scheduleData = getDefaultScheduleData();
    saveToLocalStorage();
...
function editActivity(index) {
  ...
  document.getElementById("editEnableReminder").checked =
    item.EnableReminder !== false;
